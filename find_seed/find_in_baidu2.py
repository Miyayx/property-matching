...
输入：
    1. 初步的百度template(similar_property/baidu_template.py获得), 即enwiki template与其对应的baidu attribute(该template下attribute候选集)
...
ENWIKI_TEMPLATE_BAIDU_ATTRIBUTE = os.path.join(DIR, "enwiki-template-baidu-attribute.dat4-2")
...
def find_in_baidu():
    ...
    for line in open(ENWIKI_TEMPLATE_BAIDU_ATTRIBUTE):
        ...
        if not tem.lower() in case_tem_label:
            continue
        print tem
        tlabel = case_tem_label[tem.lower()]
